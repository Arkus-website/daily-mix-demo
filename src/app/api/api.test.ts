...
describe('POST /api/mix/:id/share', () => {
  ...
  it('returns a token that resolves to the mix without a session, and reuses it on a second call', async () => {
    const { id } = await (await getToday(req('/api/mix/today', 'u_ben'))).json();

    const first = await (await share(req(`/api/mix/${id}/share`, 'u_ben', 'POST'), params(id))).json();
    expect(first.token).toEqual(expect.any(String));

    const shared = findMixByShareToken(first.token);
    expect(shared?.id).toBe(id);
    expect(shared?.userId).toBe('u_ben');

    const second = await (await share(req(`/api/mix/${id}/share`, 'u_ben', 'POST'), params(id))).json();
